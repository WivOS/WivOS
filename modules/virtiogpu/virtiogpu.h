...
typedef struct {
	...
	uint32_t width;
	uint32_t height;
} __attribute__((packed)) virtio_rect_t;
...
typedef struct {
    ...
    uint32_t height;
    uint32_t depth;
} __attribute__((packed)) virtio_box_t;

typedef struct {
	virtio_rect_t rect;
    uint32_t enabled;
    uint32_t flags;
} __attribute__((packed)) virtio_mode_t;
...
typedef struct {
    ...
    uint32_t padding;
} __attribute__((packed)) virtio_gpu_get_capset_info_req_t;
...
typedef struct {
    ...
    uint32_t padding;
} __attribute__((packed)) virtio_gpu_get_capset_info_res_t;
...
typedef struct {
    uint32_t id;
    virtio_gpu_resource_create_2d_t createCommand;
    virtio_gpu_mem_entry_t attachedBacking;
    virtgpu_set_scanout_t currentScanout;
} virtgpu_resource_t;

typedef struct {
    uint32_t id;
    virtio_gpu_context_create_t createCommand;

    uint32_t resourceAllocator;
} virtgpu_ctx_t;
...
typedef struct {
    ...
    uint32_t ctxAllocator;

    virtgpu_resource_t *currentResource;
    virtgpu_ctx_t *currentCtx;
