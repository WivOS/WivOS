...
void outb(uint16_t p, uint8_t d) {
    asm volatile("outb %0, %1" :: "a"(d), "dN"(p));
}

void outw(uint16_t p, uint16_t d) {
    asm volatile("outw %0, %1" :: "a"(d), "dN"(p));
}

void outl(uint16_t p, uint32_t d) {
    asm volatile("outl %%eax, %%dx" :: "a"(d), "dN"(p));
}

uint8_t inb(uint16_t p) {
    uint8_t retVal;
    asm volatile("inb %1, %0" : "=a"(retVal) : "dN"(p));
    return retVal;
}

uint16_t inw(uint16_t p) {
    uint16_t retVal;
    asm volatile("inw %1, %0" : "=a"(retVal) : "dN"(p));
    return retVal;
}

uint32_t inl(uint16_t p) {
    uint32_t retVal;
    asm volatile("inl %%dx, %%eax" : "=a"(retVal) : "dN"(p));
    return retVal;
}
